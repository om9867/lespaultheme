 ...
 * For Dynamic
    * only one element
    * sync element
    * Deley time b/w charcters
    * Delay time b/w two elements
    * 
 * 
 * 
 */
...
(function ($) {
  "use strict";
    ...
    var $sync_selector = '';
    var $words = 1;
    $.fn.text_animation = function(params){
        
        var $settings = $.extend($.fn.text_animation.options, params);
        
        $this = $(this);
        $(this).prepend('<span class="text_span"></span>') ;
        
        if($settings.sync_selector){
            $sync_selector = $($settings.sync_selector);
            $sync_selector.prepend('<span class="text_span"></span>') ;
        }
        $.fn.convert_into_char($this,1);
        
    }
    
    $.fn.convert_into_char = function(element,index){
            
            var $settings = $.fn.text_animation.options;
            var li_length = element.children('ul').children('li').length;   
            
            if(li_length > 0){
                    var $child_span = element.children('.text_span');
                    
                    $child_span.text('');
                    $child_span.text(element.children('ul').find('li:nth-child('+index+')').text());
                    $child_span.lettering('words');
                    $child_span.children('span[class^="word"]').lettering();
                    var char_length = $child_span.children('span[class^="word"]').children().css('visibility','hidden').length;
                    
                    $.fn.runj(char_length,element,$settings.p_el_delay);
                    
            }
    }
    var ch = 0;
    var dtime = 2000;
    $.fn.runj = function(char_length,target,delay_time){
                
                var $settings = $.fn.text_animation.options;
                
                var li_length = target.children('ul').children('li').length;
                ...
                if(ch == char_length){
                    ...
                    return delay(delay_time).then(function(){                        
                        ...
                        if($settings.sync_selector && target.hasClass($settings.sync_selector.replace('.',''))){   
                            
                            dtime = $settings.p_el_delay;
                            $words++;
                            if($words>li_length) $words = 1;
                            
                            $sync_selector.find('.text_span').html('');
                            $.fn.convert_into_char($this,$words);
                        }
                        else if(!$settings.sync_selector){
                            dtime = $settings.p_el_delay;
                            $words++;
                            if($words>li_length) $words = 1;
                            ...
                            $.fn.convert_into_char($sync_selector,$words);
                            dtime = $settings.s_el_delay;
                        }
                    });
                }
                else{
                    var mtime = setTimeout(function(){
                           
                           target.find('span[class^="word"]').find('span[class^="char"]:nth-child('+ch+')').addClass('animated '+$settings.animation_style).css({display: 'inline-block', visibility: 'visible'}).show();                           
                           $.fn.runj(char_length,target,dtime);
                           ch += 1;
                    },$settings.char_delay);
                }
